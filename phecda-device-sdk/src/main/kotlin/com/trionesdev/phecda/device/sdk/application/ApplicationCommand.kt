...
import com.trionesdev.phecda.device.contracts.errors.ErrorKind.KindContractInvalid
import com.trionesdev.phecda.device.contracts.errors.ErrorKind.KindEntityDoesNotExist
import com.trionesdev.phecda.device.contracts.errors.ErrorKind.KindServiceLocked
...
object ApplicationCommand {
    ...
    fun validateServiceAndDeviceState(deviceName: String?, dic: Container): Device? {
        ...
        if (ds?.adminState?.equals(AdminState.LOCKED) == true) {
            throw CommonPhedaException(KindServiceLocked, "service locked")
        }
        return Cache.devices()?.forName(deviceName!!)?.let {
            if (it.adminState?.equals(AdminState.LOCKED) == true) {
                throw CommonPhedaException(KindServiceLocked, String.format("device %s locked", it.name))
            }
    ...
    fun readDeviceCommand(device: Device, commandName: String, attributes: String?, dic: Container): Event? {
        Cache.profiles()?.deviceCommand(device.profileName, commandName)?.let { dc ->
            ...
            throw CommonPhedaException(KindEntityDoesNotExist, String.format("DeviceCommand %s not found", commandName))
        }
        return null
    }
